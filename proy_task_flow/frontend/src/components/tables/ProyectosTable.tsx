...
export default function ProyectosTable({ proyectos, loading, onEditar, onEliminar }: Props) {
  ...
  if (!proyectos.length) return <EmptyState icon={Users} title="Sin proyectos" description="Crea el primer proyecto para comenzar." />;
      ...
      <table className="w-full text-sm">
        <thead className="bg-slate-50 text-slate-600 font-semibold">
          <tr>
            ...
            <th className="text-left px-5 py-3">Usuario_ID</th>
        ...
        <tbody>
          {proyectos.map((u) => (
            <tr key={u.id} className="border-t border-slate-50 hover:bg-slate-50/50 transition-colors">
              ...
              <td className="px-5 py-3 text-slate-600">{u.usuarioId}</td>
