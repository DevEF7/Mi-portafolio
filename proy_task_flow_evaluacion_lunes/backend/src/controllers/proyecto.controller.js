    ...
    try {
        ...
        const proyecto = await prisma.proyecto.findUnique({
            ...
            include: {
                ...
                actualizadoPor: { select: { id:true, email: true } },
            },
        });
...
const updateProyecto = async (req, res, next) => {
    try {
        const { id } = req.params;
        const { nombre, descripcion, usuarioId, actualizadoPorId } = req.body;
        const data = {};
        ...
        if (actualizadoPorId !== undefined && actualizadoPorId !== null) {
            const editor = await prisma.usuario.findUnique({
                where: { id: Number(actualizadoPorId) },
            });
            if (!editor) {
                return res.status(404).json({ message: 'Usuario editor no encontrado' });
            }
            data.actualizadoPorId = Number(actualizadoPorId);
        }
