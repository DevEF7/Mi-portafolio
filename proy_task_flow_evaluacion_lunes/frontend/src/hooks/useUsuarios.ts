...
export const useUsuarios = () => {
  ...
  const cambiarPassword = async (id: number, payload: ActualizarUsuarioPasswordDTO) => {
    const actualizado = await usuarioService.updatePassword(id, payload);
    setUsuarios((prev) => prev.map((u) => (u.id === id ? actualizado : u)));
    return actualizado;
  };
  ...
  return { usuarios, loading, error, cargar, crear, actualizar, cambiarPassword, eliminar };
};
